...
class XENVideoModel(nn.Module):
 def __init__(self,cond_dim=384,base=32):
  super().__init__(); td=base*4; self.time=SinusoidalTimeEmbedding(td); self.cp=nn.Sequential(nn.Linear(cond_dim,cond_dim),nn.SiLU(),nn.Linear(cond_dim,cond_dim)); self.i=nn.Conv3d(3,base,3,padding=1); self.d1=VideoResBlock(base,base,td,cond_dim); self.d2=VideoResBlock(base,base*2,td,cond_dim); self.d3=VideoResBlock(base*2,base*4,td,cond_dim); self.m1=VideoResBlock(base*4,base*4,td,cond_dim); self.m2=VideoResBlock(base*4,base*4,td,cond_dim); self.u3=VideoResBlock(base*6,base*2,td,cond_dim); self.u2=VideoResBlock(base*3,base,td,cond_dim); self.u1=VideoResBlock(base*2,base,td,cond_dim); self.o=nn.Conv3d(base,3,3,padding=1)
 def forward(self,x,t,c):
